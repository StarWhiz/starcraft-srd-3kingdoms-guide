These are the visual unit combinations for all the Tier 7 Units + of the Mythics I own and other people own.

```
Credits to Mister.K for Mythics: Archon, Marine, and Ling Mythics.
Credits to Scroto.Baggins for Immortals: No.2-Lighter DT, No.4-Zealot, No.6-Hydralisk, No.8-Zerrigan
Credits to TKKTKK for Immortals: No.1-Civilian, No.5-Templar
```

Rely on the screenshots not my text descriptions because those I can make mistakes on...

# Tier 7 Units
## T7 Civilian
- 2x T5 Marines (Builds from T5 Marine)
- 1x T4 Ghost + 1x T4 Firebat
- 1x T3 Wraith + 1x T3 Dropship

![](/media/T7-Civilian.jpg)

## T7 Dark Templar
- 1x T6 SCV (Builds from T6 SCV)
- 1x T4 Dark Templar
- 2x T3 Wraith + 2x T3 Dropship
- 2x T2 Vulture

![](/media/T7-DarkTemplar.jpg)

## T7 Dark Archon
- 1x T6 Probe
- 2x T4 Queen (Builds from T4 Queen)
- 1x T3 Defiler
- 2x T2 Hydra

![](/media/T7-DarkArchon.jpg)

## T7 Ghost
- 1x T6 SCV
- 2x T4 Ghost (Builds from T4 Ghost)
- 1x T3 Probe
- 1x T3 Scout
- 2x T1 Marine

![](/media/T7-Ghost.jpg)

## T7 Zealot
- 1x T6 Probe (Builds from T6 Probe)
- 1x T4 Dark Templar + 1x T4 Dragoon
- 2x T3 Probe

![](/media/T7-Zealot.jpg)

## T7 Tassadar (Templar)
- 1x T5 Corsair (Builds from T5 Corsair) + 1x T5 Zergling
- 1x T4 Firebat + 1x T4 Dragoon
- 2x T3 Defiler

![](/media/T7-Tassadar.jpg)

## T7 Dragoon
- 1x T6 Probe
- 2x T4 Dragoon (Builds from T4 Dragoon)
- 1x T3 Scout
- 2x T2 Zealot + 2x T2 Interceptor
- 2x T1 Broodling

![](/media/T7-Dragoon.jpg)

## T7 Hydralisk
- 1x T6 Drone (Builds from T6 Drone)
- 2x T4 Queen
- 1x T3 Defiler
- 2x T2 Hydralisk + 2x T2 Scourge

![](/media/T7-Hydra.jpg)

## T7 Ultralisk
- 1x T6 Drone (Builds from T6 Drone)
- 1x T4 Firebat
- 2x T3 Scout + 2x T3 Guardian + 1x T3 Defiler

Active Ability (Q): Whole team is invulnerable for 1 second

![](/media/T7-Ultra.jpg)

## T7 Zerg Kerrigan (aka Zerrigan)
> [!NOTE]  
> **Can only have 1 Zerg Kerrigan in the game. She is a healer. The mythic medic is built from her**

- 1x T5 Bear (Builds from T5 Bear) + 1x T5 Corsair
- 1x T4 Dragoon + 1x T4 Queen
- 1x T3 Dropship + 1x T3 Scout

Active Ability (Q): Large AoE Full Heal

![](/media/T7-Kerrigan.jpg)

# Mythics

## 1. Mythic Marine
- 1x T7 Dark Templar
- 1x T5 Marine
- 2x T4 Ghost + 1x T4 Firebat
- 2x T3 Wraith
- 3x T2 Vulture
- 2x T1 Drone

Active Ability (Q): Throws down a spear that damages all units in a very large AoE where the mouse is pointed.

![](/media/mythic-marine.png)

## 2. Mythic SCV
- 1x T7 Civilian
- 1x T6 SCV
- 1x T3 Dropship
- 3x T2 SCV
- 2x T1 Broodling

Active Ability (Q): 3 Seconds of invulnerability

No Screenshot Available (I don't have it yet)

## 3. Mythic Sarah Kerrigan (aka Terrigan)
- 1x T7 Tassadar (Templar)
- 1x T6 Probe
- 2x T3 Probe (Builds from T3 Probe)
- 1x T2 Interceptor + 2x T2 Scourge
- 2x T1 Dark Templar + 2x T1 Marine

Active Ability (Q): Throws down a bunch of swords of light at a single point. Very small AoE.

![](/media/mythic-sarah-kerrigan.png)

## 4. Mythic Blue Archon
- 1x T7 Ultralisk
- 1x T5 Corsair
- 1x T5 Zergling
- 2x T3 Probe

![](/media/mythic-archon.png)

## 5. Mythic Zergling
> [!NOTE]  
> It doesn't require a T7 unit, but costs the same amount of units as any other mythics.

- 1x T6 Drone
- 2x T5 Zergling
- 2x T4 Dark Templar
- 2x T3 Guardian
- 2x T1 Zergling

Active Ability (Q): Interrupts Boss and locks him for a few seconds in a 1v1.

![](/media/mythic-zergling.jpg)

## 6. Mythic Defiler
- 1x T7 Hydralisk
- 1x T5 Corsair
- 3x T4 Queen
- 2x T3 Defiler

Active Ability (Q): Large AoE damage overtime ability. Spammable.

![](/media/mythic-defiler.jpg)

## 7. Mythic Medic
> [!NOTE]  
> Mythic Medic has no base blink. She can only blink to team mates that have a full heart icon.
> Mythic Medic shares team damage.

- 1x T7 Zerg Kerrigan
- 1x Regular Mineral Medic (Builds From Regular Mineral Medic)
- 2x T5 Bear
- 2x T2 Interceptor
- 2x T2 Scourge

Active Ability (E): From any distance blink to teammate with a full heart icon and heal them for 50% HP. Cooldown of 3 seconds. Spammable.

![](/media/mythic-medic.png)

# Immortals

## 1. Immortal Civilian
- 1x T7 Civilian
- 2x T6 SCV
- 1x T3 Dropship + 2x T3 Probe
- 1x T2 SCV
- 2x T1 Drone + 2x T1 Dark Templar + 2x T1 Zergling

![](/media/immortal-civilian.png)

## 2. Immortal Lighter Dark Templar
- 1x T7 Dark Templar + 1x T7 Ultralisk
- 1x T5 Marine
- 1x T4 Ghost
- 1x T3 Wraith
- 3x T2 Vulture
- 2x T1 Drone + 2x T1 Dark Templar

![](/media/immortal-darktemplar.png)

## 3. Immortal Dark Archon
No Data until I unlock on BETA 22 patch.

## 4. Immortal Zealot
- 1x T7 Zealot
- 1x T6 Probe
- 1x T4 Firebat + 2x T4 Dark Templar
- 2x T3 Probe
- 3x T2 Zealot
- 2x T1 Dark Templar

![](/media/immortal-zealot.png)

## 5. Immortal Templar
- 1x T7 Tassadar (Templar) + 1x T7 Dark Archon
- 1x T5 Corsair
- 1x T4 Queen
- 1x T3 Wraith
- 3x T2 Interceptors
- 2x T1 Marine + 2x T1 Broodling

![](/media/immortal-templar.png)

## 6. Immortal Hydralisk
- 1x T7 Hydralisk
- 1x T6 Drone
- 2x T5 Zergling
- 1x T3 Guardian + 1x T3 Defiler
- 3x T2 Hydralisk
- 1x T1 Drone + 1x T1 Dark Templar

![](/media/immortal-hydra.png)

## 7. Immortal Darker Dark Templar
No Data

## 8. Immortal Zerg Kerrigan (Immortal Zerrigan)
- 1x T7 Zerg Kerrigan + 1x T7 Dark Templar
- 1x T5 Bear
- 1x T4 Dragoon
- 1x T2 Interceptor + 1x T2 Hydralisk + 1x T2 Scourge

![](/media/immortal-zerrigan.png)

## Mythic Bonuses
These cost 50 stones per initial upgrade after you have your mythic unlocked. Then the stone costs increases each time.

I'll probably be going for the No.7 Upgrade, No.2 Upgrade, and No.4 Upgrade at least once first.

- No.1 - Marine: Increases damage to all skills +5%
- No.2 - SCV: 3∼4 star attack power upgrade +1
- No.3 - Sarah Kerrigan: Minerals at startup +5/+3/+2/+2/+2
- No.4 - Blue Archon: 5∼6 star attack power upgrade +1
- No.5 - Zergling: Increased probability of selling 1 star units for 3 minerals +5%
- No.6 - Defiler: Mana recovery per second +0.5
- No.7 - Medic: Probability of obtaining additional minerals from another world +30%


## Immortal Bonuses
I only have this one...

- No.3 Dark Archon: Reduced line and mission movement speed -10%
